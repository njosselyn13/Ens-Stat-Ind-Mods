...
def main(args: argparse.Namespace):
    ...
    if not os.path.isfile(progressFilePath):
        with open(progressFilePath, "w") as outputfile:
            writer = csv.DictWriter(outputfile, lineterminator='\n',
                                    fieldnames=["Epoch", "Train Loss", "Train Source Cls Acc",
                                                "Transfer Loss", "Train Cls Loss", "Val Loss", "Val Target Acc", "Test Loss", "Test Target Acc"])
            writer.writeheader()
    ...
    if args.phase == 'analysis':
        ...
        # plot t-SNE
        tSNE_filename = osp.join(logger.visualize_directory, 'TSNEc2q.png')
    ...
    if args.phase == 'test':
        ...
        np.save('feature_lists/dataset/' + 'MCC/' + str(args.source) + '_' + str(args.target) + '_' + str(args.log[-1]) + '_feature_list.npy', feat_list)
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        loss_record, cls_acc_record, trans_loss_record, cls_loss_record = train(train_source_iter, train_target_iter, classifier, mcc_loss, optimizer,
        ...
        with open(progressFilePath, "a") as outputfile:  # create csv if not already made
            writer = csv.DictWriter(outputfile, lineterminator='\n',
                                    fieldnames=["Epoch", "Train Loss", "Train Source Cls Acc",
                                                "Transfer Loss", "Train Cls Loss", "Val Loss", "Val Target Acc", "Test Loss", "Test Target Acc"])  # define headers
            writer.writerow(
                {"Epoch": epoch, "Train Loss": loss_record, "Train Source Cls Acc": cls_acc_record, "Transfer Loss": trans_loss_record, "Train Cls Loss": cls_loss_record,
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
    ...
    progress = ProgressMeter(
        ...
        [batch_time, data_time, losses, trans_losses, cls_losses, cls_accs],
    ...
    return losses.avg, cls_accs.avg, trans_losses.avg, cls_losses.avg
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', default=30, type=int, metavar='N', # changed default from 20 to 30
