...
def main(args: argparse.Namespace):
    ...
    if args.phase == 'test':
        classifier.load_state_dict(torch.load(logger.get_checkpoint_path('best'))) # use best for DomainNet, 'latest' for ARL data, originally this one was 'best' and not latest when all others were latest
        ...
        # np.save(args.log + '/' + 'feature_list.npy', feat_list) #+ str(args.source) + '_' + str(args.target) + '/'

        np.save('feature_lists/dataset/' + 'AFN/' + str(args.source) + '_' + str(args.target) + '_' + str(args.log[-1]) + '_feature_list.npy', feat_list)
        ...
        test_gt_pred.to_csv(args.log + "/" + 'dataset_test_set_gt_pred_ratings_2.csv')
        # test_gt_pred.to_csv(args.log + "/" + 'small_domainnet_train_set_gt_pred.csv')
