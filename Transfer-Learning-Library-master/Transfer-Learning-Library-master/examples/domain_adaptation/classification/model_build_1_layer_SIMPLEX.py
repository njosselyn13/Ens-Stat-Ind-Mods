...
parser.add_argument('--l1', '--lambda1', default=0.0, type=float)
...
lambda_1 = args.l1 #0.00005
lambda_2 = args.l2 #0.00005 #1e-4
...
file_save = 'lambda1' + str(lambda_1) + 'lambda2' + str(lambda_2) + 'Gaussian_' + str(gaussian_projecting) + '_Feat_' + str(feat_extract_exps) + '_SOFTMAX_' + str(sftmx) + '_stacked_linear_1layer_lr' + lr_str + '_epochs' + str(num_epochs)
...
for adapt_task in adapt_tasks:
    ...
    image_gt_labels = task_csv_train['Ground Truth'].dropna().tolist()
    ...
    # weight_check = model.net[-1].weight
    #
    # print('weight check shape:', weight_check.shape)
    #
    # sum_w = weight_check.sum(dim=0)
    # sum_w_minus = torch.ones(sum_w.shape) - sum_w
    # sum_w_minus_sq = torch.square(sum_w_minus)
    # sum_w_minus_sq_sum = sum_w_minus_sq.sum()
    # print('sum_w shape:', sum_w_minus_sq_sum.shape)
    # print('sum_w value:', sum_w_minus_sq_sum)

    # simplex_max = torch.nn.ReLU(inplace=False)
    #
    # simp_max_weights = simplex_max(-weight_check)
    #
    # print('shape simplex max weights:', simp_max_weights.shape)
    # print('max simplex max weights:', simp_max_weights.max())
    # print('min simplex max weights:', simp_max_weights.min())
    #
    # simp_max_w_sum = simp_max_weights.sum()
    #
    # print('shape sum:', simp_max_w_sum.shape)
    # print('value sum:', simp_max_w_sum)
    ...
    for epoch in range(0, num_epochs):
        ...
        for (x,y) in train_loader:
            ...
            sum_w = weight_check.sum(dim=0)
            sum_w_minus = torch.ones(sum_w.shape).to(device) - sum_w
            sum_w_minus_sq = torch.square(sum_w_minus)
            sum_w_minus_sq_sum = sum_w_minus_sq.sum()
            # print('sum_w shape:', sum_w_minus_sq_sum.shape)
            # print('sum_w value:', sum_w_minus_sq_sum)

            loss_cls = nn.functional.cross_entropy(logit, y.long()) # target y needed to be long tensor--different for diff loss functions

            # print('Cls loss:', loss.item())

            loss_norm = lambda_1 * sum_w_minus_sq_sum

            # print('Cls + simp_norm loss:', loss.item())

            loss_max = lambda_2 * simp_max_w_sum

            loss = loss_cls + loss_norm + loss_max

            # print('Cls + simp_norm + simp_max loss:', loss.item())

            # print()


            losses_epoch.append(loss.item())
            losses_epoch_cls.append(loss_cls.item())
            losses_epoch_norm.append(loss_norm.item())
            losses_epoch_max.append(loss_max.item())
        ...
        loss_epoch_cls_avg = mean(losses_epoch_cls)
        loss_epoch_norm_avg = mean(losses_epoch_norm)
        loss_epoch_max_avg = mean(losses_epoch_max)

        losses.append(loss_epoch_avg)
        losses_cls.append(loss_epoch_cls_avg)
        losses_norm.append(loss_epoch_norm_avg)
        losses_max.append(loss_epoch_max_avg)
    ...
    loss_acc_logger_csv['Cls Loss'] = pd.Series(losses_cls)
    loss_acc_logger_csv['Norm Loss'] = pd.Series(losses_norm)
    loss_acc_logger_csv['Max Loss'] = pd.Series(losses_max)
