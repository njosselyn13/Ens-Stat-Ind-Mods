...
# sys.path.append('.\\ViT-pytorch-main\\ViT-pytorch-main\\models')
...
parser.add_argument('--pseudo', default='domainnet_single_pseudo_labels_using_conf_score_train_data_analysis.csv', type=str)
...
# file_save = 'small_dnet_Gaussian_' + str(gaussian_projecting) + '_Feat_' + str(feat_extract_exps) + '_SOFTMAX_' + str(sftmx) + '_stacked_linear_1layer_lr' + lr_str + '_epochs' + str(num_epochs) + '_weights_' + str(non_neg)
# file_save = exp_id + '_pseudo_dnet_single_src_' + pseudo_type + '_' + str(gaussian_projecting) + '_Feat_' + str(feat_extract_exps) + '_SOFTMAX_' + str(sftmx) + '_stacked_linear_1layer_lr' + lr_str + '_epochs' + str(num_epochs) + '_weights_' + str(non_neg)
file_save = exp_id # + '_' + semi_sup + '_pseudo_dnet_single_src_' + pseudo_type + '_lr' + lr_str + '_epochs' + str(num_epochs) + '_beta_' + str(beta) + '_labsmooth_' + str(label_smooth) + '_' + model_train


# folder_save_name = parent_dir + file_save
folder_save_name = 'NEW_EXPS/pseudolabel_exps/dnet/single/' + file_save
...
# ###### Ignore this
# # def SemiSupervisedDataset(ground_truth_labels: List[int], pseudo_labels: List[int], fraction_gt: float = 0.25, random_seed: int = 42):
# #     ids = [f"sample{i}" for i in range(len(ground_truth_labels))]
# #
# #     # Fraction of GT to keep
# #     fraction_gt = fraction_gt
# #     # n_shots = n_shots
# #     random_seed = random_seed  # for reproducibility
# #
# #     # Step 1: Pick indices for GT
# #     np.random.seed(random_seed)
# #     num_gt = int(len(ground_truth_labels) * fraction_gt)
# #     gt_indices = np.random.choice(len(ground_truth_labels), size=num_gt, replace=False)
# #
# #     # chosen = np.random.choice(class_idxs, size=n_shots, replace=False)
# #     # gt_indices.extend(chosen)
# #
# #     # Step 2: Build new label list + type column
# #     final_labels = []
# #     label_types = []
# #
# #     for i in range(len(ground_truth_labels)):
...
# #     })
# #
# #     # df.to_csv("semi_supervised_labels.csv", index=False)
# #
# #     return df
...
class VariableTokenTransformerNet(nn.Module):
    def __init__(self, num_tokens=25, output_dim=345, d_model=256, nhead=8, num_layers=3): # d_model=256, nhead=8
...
class TokenTransformerNet(nn.Module):
    def __init__(self, num_tokens=25, feat_dim=256, output_dim=345, d_model=256, nhead=8, num_layers=3):
...
class TransformerEnsembleNet(nn.Module):
    def __init__(self, input_dim=8625, output_dim=345, d_model=512, nhead=8, num_layers=3):
...
class ViTFeatureAdapter(nn.Module):
    def __init__(self, vit_model, num_tokens=25, feat_dim=256, output_dim=345, d_model=768):
        super().__init__()
        self.num_tokens = num_tokens
        self.feat_dim = feat_dim
        self.output_dim = output_dim
...
class ens_net(nn.Module):
    def __init__(self, input_dim=8625, hidden_dim=1000, output_dim=345, model_struct='sequential', beta=0.0):
...
for adapt_task in adapt_tasks:
    ...
    if feat_extract_exps == True:
        ...
        # for t in tasks:
        models_concat = []
        #models = ['feature_lists/DANN/', 'feature_lists/JAN/', 'feature_lists/CDAN/', 'feature_lists/AFN/', 'feature_lists/MCC/']
        for m in models:
            print(m)
    ...
    if semi_sup == 'semi':
        ...
        task_csv_train_gt = pd.read_csv(
           '/home/njjosselyn/ARL/domain_adaptation/JAN_CDAN/Transfer-Learning-Library-master/Transfer-Learning-Library-master/examples/domain_adaptation/classification/train_data_analysis/Ensemble_DomainNet/ORIGINAL_SPLIT/' + adapt_task + '/afn_subset_exps/' + 'afn_subset_exps.csv')  # NEED TO FIND PATH TO CORRECT TRAIN DATA LABELS, for 1 adapt task, the images are the same across each DA model so only need to load from 1 model and just chose afn to use
        gtlist = task_csv_train_gt['Ground Truth'].dropna().tolist()

        task_csv_train_pseudo = pd.read_csv(
            '/home/njjosselyn/ARL/domain_adaptation/JAN_CDAN/Transfer-Learning-Library-master/Transfer-Learning-Library-master/examples/domain_adaptation/classification/train_data_analysis/Ensemble_DomainNet/ORIGINAL_SPLIT/' + pseudo_train_file)
    ...
    # tgtd = adapt_task[-1]
    # task_csv_train = pd.read_csv('/home/njjosselyn/ARL/domain_adaptation/DomainNet_all_correct/DomainNet_small/' + tgtd + '_DomainNet_small_labels.csv')
    ...
    # Transformer 25 tokens
        ...
        model = TokenTransformerNet(
            ...
            output_dim=output_dimension  # 345
        ).to(device)
    ...
    for epoch in range(0, num_epochs):
        ...
        if semi_sup == 'semi':
            ...
            for (x,y) in train_loader:
                ...
                # beta = 0
                if label_smooth != 0.0:
                    ce_loss = nn.functional.cross_entropy(logit, y.long(), label_smoothing=label_smooth) # target y needed to be long tensor--different for diff loss functions
                else:
