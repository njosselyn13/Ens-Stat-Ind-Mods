...
for experiment in local_folder_list:
    ...
    train_cls_loss = excel_file['Train Cls Loss'].dropna().tolist()

    log_train_cls_loss = []
    for tcl in train_cls_loss:
        log_train_cls_loss.append(math.log10(tcl))
    ...
    train_acc_tgt_list_ = [round(num, 2) for num in train_tgt_acc]  # rounded train tgt accuracies
    ...
    mode_train_tgt_acc = mode(train_acc_tgt_list_)
    ...
    p23 = plt.figure()
    plt.plot(train_cls_loss)
    plt.xlabel('Epochs')
    plt.ylabel('Cls Loss')
    plt.title('Train Cls Loss')
    plt.grid()
    plt.savefig(experiment + 'train_cls_loss.jpg')
    ...
    p53 = plt.figure()
    plt.plot(log_train_cls_loss)
    plt.xlabel('Epochs')
    plt.ylabel('Log Cls Loss')
    plt.title('Train Log Cls Loss')
    plt.grid()
    plt.savefig(experiment + 'train_cls_log_loss.jpg')
    ...
    plt.plot(train_cls_loss, '-m', label='Train Cls')
    ...
    plt.plot(log_train_cls_loss, '-m', label='Train Cls')
...
print()
print("CONFIRM ALL EPOCHS RAN FOR ALL FOLDS:") # should print # epochs you desired to run for, for each fold (i.e. 30)
